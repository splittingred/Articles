...
class modBlogPostUpdateProcessor extends modResourceUpdateProcessor {
    ...
    public function getYearParent() {
        $this->yearParent = $this->modx->getObject('modResource',array(
            ...
            'alias' => date('Y',strtotime($this->object->get('createdon'))),
        ));
        if (!$this->yearParent) {
            $date = date('Y',strtotime($this->object->get('createdon')));
    ...
    public function getMonthParent() {
        $this->monthParent = $this->modx->getObject('modResource',array(
            ...
            'alias' => date('m',strtotime($this->object->get('createdon'))),
        ));
        if (!$this->monthParent) {
            $date = date('m',strtotime($this->object->get('createdon')));
    ...
    public function getDayParent() {
        $this->dayParent = $this->modx->getObject('modResource',array(
            ...
            'alias' => date('d',strtotime($this->object->get('createdon'))),
        ));
        if (!$this->dayParent) {
            $date = date('d',strtotime($this->object->get('createdon')));
